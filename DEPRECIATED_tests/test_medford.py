...
def test_pdam_cunning():
    runMedford(
        PurePath("samples/pdam_cunning.MFD"),
        True,
        MFDMode.OTHER,
        ErrorMode.all,
        ErrorOrder.line,
...
def test_freeform():
    runMedford(
        PurePath("samples/made_up_Freeform.MFD"),
        True,
        MFDMode.OTHER,
        ErrorMode.all,
        ErrorOrder.line,
...
def test_bcodmo():
    runMedford(
        PurePath("samples/made_up_BCODMO.MFD"),
        True,
        MFDMode.BCODMO,
        ErrorMode.all,
        ErrorOrder.line,
...
def test_bagit():
    runMedford(
        PurePath("samples/bagit_example/made_up_BAGIT.MFD"),
        True,
        MFDMode.BAGIT,
        ErrorMode.all,
        ErrorOrder.line,
...
def test_no_crash_on_first_line_missing_desc(tmp_path):
    sample_txt = ["", "@MEDFORD-Version 1.0"]
...
def test_comment_only_is_considered_empty(tmp_path):
    ...
    sample_txt = ["@MEDFORD testing", "@MEDFORD-Version # I think the verison is 1.0?"]
