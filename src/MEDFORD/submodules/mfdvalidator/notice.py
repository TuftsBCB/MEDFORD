...
class MFDError(Notice):
    ...
    class Category(Enum):
        ...
        SYNTAX = (
            "syntax"  # Incorrect syntax that prevents parsing, like no DESC line to name a block.
        )
...
class MissingDescError(MFDError):
    ...
    def __init__(self, detailobj):
        ...
        if detailobj.minor_token is not None:
            ...
            raise ValueError(
                "Attempted to create a MissingDescError when the detail has no minor token, aka is a desc line. (Which means the block isn't missing a desc line.)"
            )

        msg: str = (
            f"A new block for major token {self.major_token} was created without a Name line."
        )
        help_msg: str = (
