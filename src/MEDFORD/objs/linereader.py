...
class DetailStatics:
    ...
    atat_use_regex: str = (
        "{}(?P<major>[A-Za-z_]+)-{}(?P<referenced>[A-Za-z_]+)(\\s(?P<name>.+))?$".format(
        ...
        if aa_res is None:
            ...
            if match_grps["name"] is None:
                mfdglobals.validator.add_error(
                    MissingAtAtName(match_grps["major"], match_grps["referenced"], lineno)
                )
                return None
    ...
    def process_line(line: str, lineno: int) -> Optional[Line]:
        ...
        if LineReader.is_novel_token_line(line):
            ...
            return NovelDetailLine(
                lineno, line, major_str, minor_str, rest_of_line, poss_inline, poss_tex, poss_macro
            )
